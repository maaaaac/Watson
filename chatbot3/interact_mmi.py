...
from chatbot3.dataset import MyDataset
from torch.utils.data import Dataset, DataLoader
from torch.nn import CrossEntropyLoss
from sklearn.model_selection import train_test_split
from chatbot3.train import create_model
...
def set_interact_args():
    ...
    parser.add_argument('--model_config', default='config/model_config_dialogue_small.json', type=str, required=False,
                        help='模型参数')
    parser.add_argument('--log_path', default='chatbot3/data/interacting_mmi.log', type=str, required=False,
                        help='interact_mmi日志存放位置')
    parser.add_argument('--voca_path', default='chatbot3/vocabulary/vocab_small.txt', type=str, required=False, help='选择词库')
    parser.add_argument('--dialogue_model_path', default='chatbot3/dialogue_model_40epoch', type=str, required=False,
                        help='dialogue_model路径')
    parser.add_argument('--mmi_model_path', default='/Users/Mac/.pyenv/versions/3.6.11/envs/finale/chatbot3/mmi_model_40epoch', type=str, required=False, help='互信息mmi_model路径')
    parser.add_argument('--save_samples_path', default="chatbot3/sample/", type=str, required=False, help="保存聊天记录的文件路径")
    ...
    parser.add_argument('--no_cuda', action='store_false', help='不使用GPU进行预测')
...
def chatbotmain():
    ...
    with open('/Users/Mac/.pyenv/versions/3.6.11/envs/finale/hear.txt') as f:
        speech = f.read()
    text = speech
    if args.save_samples_path:
...
if __name__ == '__main__':
    chatbotprep()
    chatbotmain()
#本文件有while true循环以保证对话机器人的持续运转，但是为了在新的python文件里融合其他的模块，把循环部分删除了，如果想要再次单独使用此文件，请重新写入循环